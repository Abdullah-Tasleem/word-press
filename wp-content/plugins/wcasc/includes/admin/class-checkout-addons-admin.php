				...
				<div class="wcasc-grid">
					...
					<div class="wcasc-field">
						...
						<select id="wcasc_source" name="wcasc_source">
							...
							<option value="related_cart" <?php selected($source, 'related_cart'); ?>><?php esc_html_e('Related products from cart', 'wcasc'); ?></option>
						</select>
					</div>

					<p id="wcasc_related_note" class="wcasc-field description" style="display:none;">
						<?php esc_html_e('When selected, the add-on will automatically show products that are related to items in the customer\'s cart (by matching product categories). Admin doesn\'t need to select categories manually.', 'wcasc'); ?>
					</p>
				</div>
		...
		<script>
			jQuery(function($) {
				function toggleSourceFields() {
					...
					$('#wcasc_related_note').toggle(src === 'related_cart');
				}
				...
				$('#wcasc_product_ids, #wcasc_category_ids').on('change', function() {
					updateCount(this.id);
				});
...
<?php
	...
	public function save_meta($post_id)
	{
		...
		// ✅ Updated save block
		$source = in_array($_POST['wcasc_source'] ?? 'products', array('products', 'categories', 'related_cart'), true) ? $_POST['wcasc_source'] : 'products';
		update_post_meta($post_id, '_wcasc_source', $source);

		if ('related_cart' === $source) {
			// Clear manual selections to avoid confusion
			update_post_meta($post_id, '_wcasc_product_ids', '');
			update_post_meta($post_id, '_wcasc_category_ids', '');
		} else {
